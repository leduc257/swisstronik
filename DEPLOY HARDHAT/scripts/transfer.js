...
async function main() {
  ...
  const replace_contractAddress = "0x011c2C22C4ba1f31A27F745FbC6DDEDFba307120";

  // Get the signer (your account)
  const [signer] = await hre.ethers.getSigners();

  // Create a contract instance
  const replace_contractFactory = await hre.ethers.getContractFactory("KYOSHU");
  const contract = replace_contractFactory.attach(replace_contractAddress);

  // Send a shielded transaction to execute a transaction in the contract
  const replace_functionName = "transfer";
  const replace_functionArgs = ["0x011c2C22C4ba1f31A27F745FbC6DDEDFba307120", "100"];
